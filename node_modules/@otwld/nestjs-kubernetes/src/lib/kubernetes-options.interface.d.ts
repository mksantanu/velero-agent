...
import { Cluster, ConfigOptions, User } from '@kubernetes/client-node/dist/config_types';
import { LoadFrom } from './load-from.enum';
export type KubernetesProviderOptions = LoadFromOptions & {
    name: string | symbol;
    loadFrom: LoadFrom;
};
export type KubernetesModuleOptions = Array<KubernetesProviderOptions> | {
    servers: Array<KubernetesProviderOptions>;
    isGlobal?: boolean;
};
export interface KubernetesModuleOptionsFactory {
    createKubernetesOptions(): Promise<LoadFromOptions> | LoadFromOptions;
}
export interface KubernetesProviderAsyncOptions extends Pick<ModuleMetadata, 'imports'> {
    useExisting?: Type<KubernetesModuleOptionsFactory>;
    useClass?: Type<KubernetesModuleOptionsFactory>;
    useFactory?: (...args: any[]) => Promise<LoadFromOptions> | LoadFromOptions;
    inject?: any[];
    extraProviders?: Provider[];
    name: string | symbol;
}
export type KubernetesModuleAsyncOptions = Array<KubernetesProviderAsyncOptions> | {
    servers: Array<KubernetesProviderAsyncOptions>;
    isGlobal?: boolean;
};
export type LoadFromOptions = LoadFromFileOptions | LoadFromStringOptions | LoadFromClusterOptions | LoadFromDefaultOptions | LoadFromOptionsOptions | LoadFromClusterAndUserOptions;
...
//# sourceMappingURL=kubernetes-options.interface.d.ts.map