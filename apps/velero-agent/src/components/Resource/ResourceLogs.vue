<template>
  <div class="grid grid-cols-1 xl:gap-4">
    ...
    >
      ...
      <div class="h-[600px]">
        <Logs :data="data" :loading="loading" />
      </div>
    </div>
  </div>
</template>
...
import { type V1DownloadTargetKind } from '@velero-agent/velero';
import { useI18n } from 'vue-i18n';
import { type Log } from '@velero-agent/shared-types';
import Logs from '@velero-agent-app/components/Log/Logs.vue';

const { t } = useI18n();
defineProps({
  data: { type: Array as PropType<Log[]>, required: true },
