<template>
  <button
    ...
    @click="redirect()"
  >
    <FontAwesomeIcon :icon="faGoogle" class="!w-4 !h-4 mr-2" />
    {{ t('auth.federated.button.title', { strategy: 'Google' }) }}
    <FontAwesomeIcon
      v-if="loading"
      :icon="faCircleNotch"
...
import { useAuth } from '@velero-agent-app/composables/auth/useAuth';
...
const redirect = () => {
  loading.value = true;
  const state = uuidv4();
  localStorage.setItem('auth.google.state', state);
